...
- **No vibrato / dynamics analysis yet** — vibrato is survived (the median absorbs it), not described.
- **No harmonic context** — it doesn't know whether a note is a leading tone or sits inside a chord, so the target never moves with the music. This is the widest gap between what it measures and what a musician hears.
- **Audio only** — no bow angle, posture, or finger position; every technique hypothesis is inferred from pitch alone.

The capability map, known gaps, and roadmap are in `references/DESIGN.md`. Read it only if the user asks where the project is headed or wants to extend the analyzer.
