...
└── references/
    └── DESIGN.md       # what it measures, known gaps, roadmap — loaded on demand
```
...
See [`references/DESIGN.md`](references/DESIGN.md) for the full design, known gaps, and roadmap. In short: monophonic only (double stops produce garbage), judged against one stated reference grid at a time, no vibrato or dynamics description yet, and no harmonic context — the tool doesn't know whether a note is a leading tone or sits inside a chord, which is the widest gap between what it measures and what a musician hears.
